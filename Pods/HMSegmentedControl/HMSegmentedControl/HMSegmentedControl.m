...
#define gray_Color [UIColor colorWithRed:0.0f/255.0f green:80.0f/255.0f blue:82.0f/255.0f alpha:1]
...
- (void)commonInit {
    ...
    self.opaque = NO;
    _selectionIndicatorColor = [UIColor colorWithRed:255.0f/255.0f green:121.0f/255.0f blue:0.0f/255.0f alpha:1.0f];
...
- (CGSize)measureTitleAtIndex:(NSUInteger)index {
    ...
    if ([title isKindOfClass:[NSString class]] && !self.titleFormatter) {
        NSDictionary *titleAttrs = selected ? [self resultingSelectedTitleTextAttributes] : [self resultingTitleTextAttributes:NO];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        ...
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            /************************Image and Text Rect Reguler code ************/
            //Change Text and image icon Y postion .
            //This will show Text top of the image  ICON
            ...
            // ------------------------------
            //              2X     3X
            //ipad   H-80   50-50   100-100
            //iphone H-50   60-30   90-30
            
            //-------------------------------
            
            /*--------------------Image and Text Rect Recalculated modified code --------------------*/
            //CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f);
            //Change Text and image icon Y postion .
            CGRect imageRect = CGRectMake(imageXOffset,2, imageWidth, imageHeight);
            CGRect textRect = CGRectMake(textXOffset, imageRect.origin.y+imageRect.size.height, textWidth, stringHeight);
            //Title chages imageRect 0
            // Fix rect position/size to avoid blurry labels
            textRect = CGRectMake(ceilf(textRect.origin.x), ceilf(textRect.origin.y), ceilf(textRect.size.width), ceilf(textRect.size.height));
            /*********************************************/
...
- (NSDictionary *)resultingTitleTextAttributes : (BOOL)flag {
    //Change font selection color
    //Method with param BOOL:flag
    NSDictionary *defaults;
    defaults = @{
                 NSFontAttributeName : [UIFont systemFontOfSize:iPad?20.0f:12.0f],
                 NSForegroundColorAttributeName :flag?gray_Color:[UIColor darkGrayColor],
                 };
...
- (NSDictionary *)resultingSelectedTitleTextAttributes {
    NSMutableDictionary *resultingAttrs = [NSMutableDictionary dictionaryWithDictionary:[self resultingTitleTextAttributes:YES]];
